...
def convex_cloud_cluster(inputFile, capacity, distances_to_arena_check, bus_names_check, matrix, choice='random'):
...
def convex_sequence_cluster(inputFile, capacity, distances_to_arena_check, bus_names_check, matrix, choice='random'):
    cluster_nodelist_dict = defaultdict(list)
    cluster_number = 0  # cluster number
    while len(bus_names_check) != 0:  # iterate till all nodes are assigned to clusters
        people_assigned = 0
...
def convex_a_star_cluster(inputFile, capacity, distances_to_arena_check, bus_names_check, matrix, k=3, choice='random'):
    cluster_nodelist_dict = defaultdict(list)
    cluster_number = 0  # cluster number
    while len(bus_names_check) != 0:  # iterate till all nodes are assigned to clusters
        people_assigned = 0
...
def cloud_cluster(inputFile, capacity, distances_to_arena_check, bus_names_check, matrix):
    cluster_nodelist_dict = defaultdict(list)
    cluster_number = 0  # cluster number
    while len(bus_names_check) != 0:#iterate till all nodes are assigned to clusters
        people_assigned = 0
...
def sequence_cluster(inputFile, capacity, distances_to_arena_check, bus_names_check, matrix):
    cluster_nodelist_dict = defaultdict(list)
    cluster_number = 0  # cluster number
...
def a_star_cluster(inputFile, capacity, distances_to_arena_check, bus_names_check, matrix, k):
    cluster_nodelist_dict = defaultdict(list)
    cluster_number = 0  # cluster number
    while len(bus_names_check) != 0:
        people_assigned = 0
...
def a_star_k_next_cluster(inputFile, capacity, distances_to_arena_check, bus_names_check, matrix, k = 3):
    cluster_nodelist_dict = defaultdict(list)
    cluster_number = 0  # cluster number
    while len(bus_names_check) != 0:
        people_assigned = 0
...
def runCluster(src, method, scenario, capacity):
    ...
    if method == "CONVEX_HULL_CLOUD_random":
        dict_clusters = convex_cloud_cluster(inputFile, capacity,
                                                     distances_to_arena_check,
                                                     bus_names_check, matrix,
                                                     choice='random')
    elif method == "CONVEX_HULL_CLOUD_distance":
        dict_clusters = convex_sequence_cluster(inputFile, capacity,
                                                        distances_to_arena_check,
                                                        bus_names_check, matrix,
                                                        choice='distance')
        ...
        dict_clusters = convex_a_star_cluster(inputFile, capacity,
                                                      distances_to_arena_check,
        ...
        dict_clusters = sequence_cluster(inputFile, capacity,
                                                      distances_to_arena_check,
                                                      bus_names_check, matrix)

    elif method == "CLOUD":
        dict_clusters = cloud_cluster(inputFile, capacity,
                                                      distances_to_arena_check,
                                                      bus_names_check, matrix)

    elif method == "A_STAR":
        dict_clusters = a_star_cluster(inputFile, capacity,
                                                      distances_to_arena_check,
                                                      bus_names_check, matrix, k=3)
        
    elif method == "A_STAR_K_NEXT":
        dict_clusters = a_star_k_next_cluster(inputFile, capacity,
                                                      distances_to_arena_check,
                                                      bus_names_check, matrix, k=3)    
